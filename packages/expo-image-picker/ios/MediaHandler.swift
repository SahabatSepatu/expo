...
internal struct MediaHandler {
  ...
  internal func handleMedia(_ mediaInfo: MediaInfo) async throws -> AssetInfo {
    let mediaType: String? =
      mediaInfo[UIImagePickerController.InfoKey.mediaType] as? String
  ...
  internal func handleMultipleMedia(_ selection: [PHPickerResult]) async throws
    -> [AssetInfo]
  {
  ...
  private func handleImage(mediaInfo: MediaInfo) async throws -> AssetInfo {
    do {
      ...
      let fileWasCopied =
        !imageModified
        && ImageUtils.tryCopyingOriginalImageFrom(
          mediaInfo: mediaInfo, to: targetUrl)
      if !fileWasCopied {
      ...
      let base64 =
        ...
        ? try ImageUtils.readBase64From(
          imageData: imageData, orImageFileUrl: targetUrl,
          tryReadingFile: fileWasCopied) : nil

      let exif =
        options.exif ? await ImageUtils.readExifFrom(mediaInfo: mediaInfo) : nil
  ...
  private func handleImage(from selectedImage: PHPickerResult) async throws
    -> AssetInfo
  {
    ...
    if fastPath {
      ...
      if let targetUrl = try? await withCheckedThrowingContinuation({
        ...
        itemProvider.loadFileRepresentation(
          ...
          guard let srcUrl = url else {
            return continuation.resume(
              throwing: error ?? FailedToReadImageException())
          }
          do {
            let destUrl = try generateUrl(
        ...
        let exif =
          options.exif
          ? (rawData.flatMap { ImageUtils.readExifFrom(data: $0) }) : nil

        log.info(
          "expo-image-picker: Using fast path for image – no slow path available."
        )
    ...
    log.info(
      "expo-image-picker: Using slow path for image – no fast path available.")
  ...
  private func handleLivePhoto(from selectedImage: PHPickerResult) async throws
    -> AssetInfo
  {
    let itemProvider = selectedImage.itemProvider
    let livePhotoObject = try await itemProvider.loadObject(
      ofClass: PHLivePhoto.self)
    ...
    let (photoUrl, pairedVideoUrl) = try generatePairedUrls(
      ...
      videoFileExtension: pairedVideoFileExtension)
    ...
    let pairedVideoAssetInfo = try getPairedAssetInfo(
      from: videoResource, fileUrl: pairedVideoUrl,
      assetId: selectedImage.assetIdentifier)
  ...
  func handleVideo(mediaInfo: MediaInfo) async throws -> AssetInfo {
    guard let pickedVideoUrl = VideoUtils.readVideoUrlFrom(mediaInfo: mediaInfo)
    else {
    ...
    if options.videoExportPreset == .passthrough {
      log.info(
        "expo-image-picker: Using fast path for legacy video – passthrough without transcoding."
      )
    }
    ...
    let videoUrlToReadDurationFrom =
      self.options.allowsEditing ? pickedVideoUrl : targetUrl
  ...
  private func handleVideo(from selectedVideo: PHPickerResult) async throws
    -> AssetInfo
  {
    ...
    if #available(iOS 15.0, *) {
      if let assetId = selectedVideo.assetIdentifier,
        ...
        let fastUrl =
          try? await VideoUtils.tryCopyingOriginalVideoFromMetadataOnlyAsset(
            assetId: assetId, fileSystem: fs)
      {
        ...
        return try buildVideoResult(
          for: fastUrl, withName: fileName, mimeType: mimeType, assetId: assetId
        )
      }
    }
    ...
    if isPassthrough {
      log.info(
        "expo-image-picker: Using fast path for video – passthrough without transcoding."
      )
      finalUrl = videoUrl
    } else {
      log.info(
        "expo-image-picker: Slow path for video – transcoding with preset \(options.videoExportPreset)"
      )
      // Create destination url for transcoded video
      let transcodedUrl = try generateUrl(
        withFileExtension: transcodeFileExtension)
    ...
    let fileName = selectedVideo.itemProvider.suggestedName.map {
      $0 + transcodeFileExtension
    }

    return try buildVideoResult(
      for: finalUrl, withName: fileName, mimeType: mimeType,
      assetId: selectedVideo.assetIdentifier)
  }

  // MARK: - utils

  private func replaceFileExtension(fileName: String, targetExtension: String)
    -> String
  {
  ...
  private func generateUrl(withFileExtension: String) throws -> URL {
    ...
    let path = fileSystem.generatePath(
      inDirectory: directory, withExtension: withFileExtension)
    return URL(fileURLWithPath: path)
  }

  private func generatePairedUrls(
    photoFileExtension: String, videoFileExtension: String
  ) throws -> (URL, URL) {
    ...
    let videoUrl = baseUrl.appendingPathComponent(filename)
      .appendingPathExtension(parsedVideoFileExtension)
    return (photoUrl, videoUrl)
  }

  private func buildVideoResult(
    for videoUrl: URL, withName fileName: String?, mimeType: String?,
    assetId: String?
  ) throws -> AssetInfo {
...
extension PHAssetResourceManager {
  fileprivate func requestData(
    ...
    return try await withCheckedThrowingContinuation { continuation in
      ...
      self.requestData(
        for: assetResource, options: options, dataReceivedHandler: dataHandler,
